...
#[derive(serde::Deserialize)]
 ...
 fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    .filter(|x| x.info.token_amount.decimals == 0 && x.info_token_amount.ui_amount == 1.0)
    .map(|x| x.info.mint)
    .collect();
 ...
 fn get_token_accounts(
    rpc: &RpcClient,
    owner &Pubkey,
 ) -> Result<Vec<solana_client::rpc_response::RpcKeyedAccounnts>, Box<dyn std::error::Error>> {
