...
fn  main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mint_account_pubkey: Pubkey = env.mint_accounnt_pubkey.parse()?;
    ...
    let assoc_instruction = spl_associated_token_account::get_associated_token_account::create_associated_token_account(
        &signer_wallet.pubkey();
        &receiver_pubkey,
        &mint_account_pubkey,
    );
    
    let mint_to_instruction: Instruction = spl_token::instuction::mint_to(
        &spl_token::ID,
        &mint_account_pubkey,
        &assoc,
        &signer_wallet.pubkey();
    ...
    let recent_blockhash = client.get_lastest_blockhash()?;
